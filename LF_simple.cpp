...
double a, G_dim, grav; //semi-major axis, period
...
vector <double> centrifugal_term; //centrifugal force pre term
vector <double> r0, v0, a0; //initial position, velocity and acceleration vectors
...
double semimajor(double period) { //function to evaluate semi-major axis value, period in days, a in AU
   ...
   a = pow(7.496e-6 * Mstar_sun * pow(Period_days, 2.0), (1.0/3.0)); //in AU
   return a*AU_to_m; //output in meters
}
...
double acceleration(double r, double grav_scalar, double centrifugal) { //acceleration function, from newton's first
    accel_new =  -grav_scalar * r + centrifugal * r;
    return accel_new;
}
...
vector <double> LF_solver(vector < vector <double> > pos, vector < vector <double> > vel, vector < vector <double> > accel,\
    ...
    for (unsigned int i = 0; i < 1000; i++) {
        ...
        //constant in front of grav force
        grav = grav_scalar(pos[i][0], pos[i][1], pos[i][2]);
        
        //new x coordinates
        x_new = position(pos[i][0], vel[i][0], accel[i][0]);
        vxdot_next = acceleration(x_new, grav, centrifugal_term[0]);
        ...
        vydot_next = acceleration(y_new, grav, centrifugal_term[1]);
        ...
        vzdot_next = acceleration(z_new, grav, centrifugal_term[2]);
...
int main() {
    ...
    centrifugal_term.push_back(-4.0* pow(PI, 2.0));
    centrifugal_term.push_back(-4.0* pow(PI, 2.0));
    centrifugal_term.push_back(0.0);
    
    
    a0.push_back(acceleration(x0, gravity_0, centrifugal_term[0]));
    a0.push_back(acceleration(y0, gravity_0, centrifugal_term[1]));
    a0.push_back(acceleration(z0, gravity_0, centrifugal_term[2]));
