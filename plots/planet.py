...
data = np.fromfile("/lustre/astro/bmce/Dusty_Tails/simulations/Mg08Fe12SiO4/nov22/KIC1255b/sdist_1.75_mdot2.0_sph_t3.0/output_struct.bin", dt)
df = pd.DataFrame(data)

i = 0
for t in df['time'].unique():
    test_df = df[df.time == t]
    ...
    for x in test_df['x']:
        ...
        if (r<0.90):
            count +=1
        i+=1
    print('At time ', t , count, ' particles were closer than 0.95a in ', npart, ' particles')
