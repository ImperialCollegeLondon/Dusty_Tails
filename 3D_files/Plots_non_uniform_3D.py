...
for i in factor_two(50):

    """#DENSITY (uniform grid) construct dataset to plot
    x = []
    y = []
    NRu.append(i)
...
"""
    ...
    dataset = open('/Users/annawilson/Documents/GitHub/Dusty_Tails/3D_files/opticaldepth.txt','r')

    flat_t = np.loadtxt(dataset)

    """for line in dataset:
        values = line.strip()
        flat_t.append(values)"""

    ThreeDt = np.reshape(flat_t,(NP,NR,NT),order='C')
...
#             for j in range(11):
#                 for k in range(11):
#                     p[i] = float(values[i + (11 * j) + (11 * 11 * k)])
#                     q[j] = float(values[i + (11 * j) + (11 * 11 * k)])
#                     r[k] = float(values[i + (11 * j) + (11 * 11 * k)])
#         # p.append(float(P))
#         # q.append(float(Q))"""
#
#     """dataset.close()
#     plt.figure()
#     plt.scatter(p,q,r,c='r',marker='o') #numerical density
#     plt.plot(z,g) #analytical gaussian function
#     plt.xlabel("Rb/a")
#     plt.ylabel("Density")
#     plt.xlim(0.,2.0)
#     plt.ylim(0.,2.0e-14)
#     plt.title("Density as a function of Rb (gaussian grid) for NR=10")
#     plt.savefig("density_%s.png" % i)
#
#
#     #OPTICAL DEPTH new dataset
#     r = []
